import time
import random
import requests
from datetime import datetime, timedelta
from threading import Thread
from flask import Flask

app = Flask('')

@app.route('/')
def home():
    return "Quotex Ultra Real-Time Engine 2026 Live"

def run_web_server():
    app.run(host='0.0.0.0', port=8080)

# --- CONFIGURATION ---
TELEGRAM_BOT_TOKEN = "8805973093:AAHnKIMb-5Mnr0yI0XR3-gIW5oUOQyLNfRA"  
TELEGRAM_CHAT_ID = "8240647626"      

# RISK MANAGEMENT CONFIG
STARTING_TRADE_AMOUNT = 10  # Base Trade ($ ya ₹)

# SAARE EXACT QUOTEX PAIRS
QUOTEX_EXACT_PAIRS = [
    "USD/BRL (OTC)", "CAD/CHF (OTC)", "NZD/CHF (OTC)", "USD/MXN (OTC)", "USD/PKR (OTC)",
    "USD/ZAR (OTC)", "EUR/USD", "NZD/JPY (OTC)", "USD/NGN (OTC)", "EUR/JPY",
    "EUR/NZD (OTC)", "GBP/NZD (OTC)", "USD/EGP (OTC)", "AUD/JPY", "GBP/USD",
    "USD/DZD (OTC)", "EUR/AUD", "EUR/GBP", "USD/INR (OTC)", "AUD/USD",
    "GBP/JPY", "NZD/USD (OTC)", "USD/BDT (OTC)", "USD/CAD", "USD/JPY",
    "USD/COP (OTC)", "CAD/JPY", "EUR/CAD", "GBP/AUD", "GBP/CAD",
    "USD/CHF", "AUD/CAD", "AUD/CHF", "USD/IDR (OTC)", "AUD/NZD (OTC)",
    "CHF/JPY", "NZD/CAD (OTC)", "USD/ARS (OTC)", "USD/PHP (OTC)", "EUR/CHF", "GBP/CHF"
]

# SESSION STATS TRACKER
stats = {
    "total_signals": 0,
    "direct_wins": 0,
    "mtg_wins": 0,
    "losses": 0
}

def send_to_telegram(message):
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "Markdown"}
    try:
        response = requests.post(url, json=payload)
        return response.json()
    except Exception as e:
        print(f"Telegram Delivery Error: {e}")
        return None

def get_real_ist_time():
    """Quotex Live Clock Engine (IST Time)"""
    return (datetime.utcnow() + timedelta(hours=5, minutes=30)).strftime("%H:%M:%S")

def track_and_send_result(pair, direction):
    global stats
    time.sleep(60)  # Wait for 1-Minute Candle Expiry
    
    # Live algorithmic evaluation simulation
    outcome = random.choice(["DIRECT_WIN", "DIRECT_WIN", "MTG_REQUIRED", "LOSS"])
    ist_now = get_real_ist_time()
    
    if outcome == "DIRECT_WIN":
        stats["direct_wins"] += 1
        result_msg = (
            f"🎯 **RESULT FOR {pair}**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🏁 **Status:** 🟢 **DIRECT SHURESHOT WIN !!**\n"
            f"⏰ **Time:** `{ist_now}`\n"
            f"🎉 Balance secured. Trend pattern validated successfully!"
        )
        send_to_telegram(result_msg)
        
    elif outcome == "MTG_REQUIRED":
        mtg_amount = STARTING_TRADE_AMOUNT * 2
        result_msg = (
            f"⚠️ **ATTENTION FOR {pair}**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🔄 **Status:** 🔴 Main Trade Loss.\n"
            f"👉 **ACTION:** **Take 1-Step MTG (Martingale)** immediately!\n"
            f"💰 **MTG Investment:** `${mtg_amount}`\n"
            f"⏰ **Time:** `{ist_now}`"
        )
        send_to_telegram(result_msg)
        
        time.sleep(60)  # Wait for MTG Expiry
        mtg_outcome = random.choice(["MTG_WIN", "TOTAL_LOSS"])
        ist_mtg = get_real_ist_time()
        
        if mtg_outcome == "MTG_WIN":
            stats["mtg_wins"] += 1
            result_msg = (
                f"🎯 **MTG RESULT FOR {pair}**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🏁 **Status:** 🟡 **MTG-1 SUCCESS WIN !!**\n"
                f"⏰ **Time:** `{ist_mtg}`\n"
                f"✅ Loss recovered + profit extracted."
            )
        else:
            stats["losses"] += 1
            result_msg = (
                f"❌ **FINAL RESULT FOR {pair}**\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🏁 **Status:** 💀 **TOTAL LOSS (Bad Market)**\n"
                f"⏰ **Time:** `{ist_mtg}`\n"
                f"🛑 Avoid trading on this pair for the next session."
            )
        send_to_telegram(result_msg)
        
    else:
        stats["losses"] += 1
        result_msg = (
            f"❌ **RESULT FOR {pair}**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🏁 **Status:** 💀 **DIRECT LOSS**\n"
            f"⏰ **Time:** `{ist_now}`\n"
            f"📉 High volatility breakout spike detected."
        )
        send_to_telegram(result_msg)

def report_scheduler():
    global stats
    while True:
        time.sleep(1800)  # 30 Minutes Interval Loop
        
        total = stats["total_signals"]
        wins = stats["direct_wins"] + stats["mtg_wins"]
        losses = stats["losses"]
        win_rate = (wins / total * 100) if total > 0 else 0
        
        report_template = (
            f"📊 **📊 QUOTEX 30-MIN SESSION SUMMARY 📊**\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"⏰ **Timestamp (IST):** `{get_real_ist_time()}`\n"
            f"📡 **Signals Processed:** `{total}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🟢 **Direct Wins:** `{stats['direct_wins']}`\n"
            f"🟡 **MTG-1 Wins:** `{stats['mtg_wins']}`\n"
            f"🔴 **Session Losses:** `{losses}`\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🏆 **Session Win Rate:** `{round(win_rate, 2)}%`\n"
            f"⚡ **Status:** {'🔥 PROFITABLE SESSION' if win_rate >= 75 else '⚠️ HIGH VOLATILITY SLOWDOWN'}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🔄 *Stats cleared. Refreshing engine for next 30 mins...*"
        )
        send_to_telegram(report_template)
        # Flush Stats for next window
        stats = {"total_signals": 0, "direct_wins": 0, "mtg_wins": 0, "losses": 0}

def start_scanner():
    global stats
    print(f"[{get_real_ist_time()}] Scanning Multi-Asset Price Feeds...")
    for pair in QUOTEX_EXACT_PAIRS:
        # High-Performance Indicator Simulation
        rsi = random.uniform(10, 90)
        trend = random.choice(["UPTREND", "DOWNTREND", "SIDEWAYS"])
        volume = random.uniform(20, 100)
        
        # Strict Shureshot Entry Filters
        if rsi < 24 and trend == "UPTREND" and volume > 78:
            direction = "🔺 CALL / UP"
            strategy = "S1-Price Action Support Reversal"
            win_rate = round(random.uniform(93.5, 98.9), 2)
        elif rsi > 76 and trend == "DOWNTREND" and volume > 78:
            direction = "🔻 PUT / DOWN"
            strategy = "S2-Price Action Resistance Reversal"
            win_rate = round(random.uniform(93.5, 98.9), 2)
        else:
            continue
            
        stats["total_signals"] += 1
        real_time = get_real_ist_time()
        
        signal_template = (
            f"🔥 **🔥 QUOTEX REAL-TIME SHURESHOT ALERT 🔥**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🚀 **Asset:** `{pair}`\n"
            f"⏱️ **Duration:** `1 MINUTE`\n"
            f"⏰ **Exact Entry (IST):** `{real_time}`\n"
            f"🎯 **Direction:** **{direction}**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"💵 **Recommended Trade:** `${STARTING_TRADE_AMOUNT}`\n"
            f"📊 **Strategy Logic:** `{strategy}`\n"
            f"💎 **Algorithmic Confidence:** `{win_rate}%`\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"⚡ *Verify candle rejection on Quotex chart before entry!*"
        )
        
        print(f"-> Pushing Verified Alert for {pair}...")
        send_to_telegram(signal_template)
        
        # Background Evaluation Thread
        Thread(target=track_and_send_result, args=(pair, direction)).start()
        time.sleep(5.0)

if __name__ == "__main__":
    Thread(target=run_web_server).start()
    Thread(target=report_scheduler).start()
    
    while True:
        start_scanner()
        time.sleep(30)
