...
# Handle flexible imports from api_quotex to bypass structural name differences
try:
    from api_quotex.client import QuotexClient
except ImportError:
    try:
        from api_quotex import QuotexClient
    except ImportError:
        try:
            from api_quotex.client import Client as QuotexClient
        except ImportError:
            from api_quotex import Client as QuotexClient
...
async def main():
    ...
    while True:
        ...
        if not my_ssid:
            logging.error("QUOTEX_SSID Environment Variable nahi mila!")
